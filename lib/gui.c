...
void compress_click()
{
    ...
    if(input_file && strlen(output_name) > 0)
    {
        gtk_label_set_text(progress_label, "Compressing...");
        
        /* Make filepath */
        size_t len_parent = strlen(parent_path);
        size_t len_output = strlen(output_name);
        ...
        else
        {
            ...
            {
                /* Huffman */
                return_status = execl("huffman_compression/bin/run", "run", "-c", "-i", input_path, "-o", output_path, NULL);
                if(return_status == -1)
            ...
            {
                /* LZW */
                return_status = execl("lzw_compression/bin/run", "run", "-c", "-i", input_path, "-o", output_path, NULL);
                if(return_status == -1)
                    err_sys("Process error");
            }
        }
...
void decompress_click()
{
    ...
    if(input_file && strlen(output_name) > 0)
    {
        ...
        size_t len_parent = strlen(parent_path);
        size_t len_output = strlen(output_name);
        char * output_path = calloc(sizeof(char), len_parent + len_output + 1);
        if(output_path == NULL)
            err_sys("Allocating memory for output_path.");
        ...
        else
        {
            ...
            {
                /* Huffman */
                return_status = execl("huffman_compression/bin/run", "run", "-d", "-i", input_path, "-o", output_path, NULL);
                if(return_status == -1)
                    err_sys("Process error");
            }
            else
            {
                /* LZW */
                return_status = execl("lzw_compression/bin/run", "run", "-d", "-i", input_path, "-o", output_path, NULL);
                if(return_status == -1)
                    err_sys("Process error");
            }
        }
